...
export default function ResultsPage() {
  const [stats, setStats] = useState<ResultStats>({ total: 0, won: 0, lost: 0, pending: 0, winRate: 0 });
  const [history, setHistory] = useState<PredictionEntry[]>([]);
  const [betBuilders, setBetBuilders] = useState<BetBuilderEntry[]>([]);
    ...
    <>
      ...
      {loading ? (
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: 'calc(100vh - 80px)', flexDirection: 'column', gap: '20px' }}>
          <h2 style={{ color: 'var(--text-primary)', fontSize: '1.5rem', fontWeight: 600 }}>Chargement des resultats...</h2>
        </div>
      ) : (
